...
void Decoder::leal(const Instruction *instruction, const Registers *registers)
{
  ...
  ptr = strtok(NULL, " ");
  //Registers::RegName reg = (Registers::RegName)registers->stringToRegNum(ptr);
  *operand2 = atoi(ptr) + registers->get((Registers::RegName)registers->
              stringToRegNum(ptr));
}  // leal()
...
void Decoder::leave(Registers *registers, const Memory &memory) const
{
  ...
  const Data &data = dynamic_cast<const Data&>(memory[registers->get(Registers::esp)]);
